...
const std::set<Era>& JsonParser::getEras() const { return m_erasArray; }

const std::set<Art>& JsonParser::getArts() const { return m_artsArray; }
...
const std::set<Art> JsonParser::getArts(const Era& era) const {
  ...
  while (it != m_artsArray.end()) {
    if (it->eraName == era.eraName) arts.insert(*it);
    ++it;
  }
  return arts;
}
...
void JsonParser::prepareErasArray() {
  ...
  for (auto eraObj : erasArray) {
    Era era(eraObj.toObject().value("eraName").toString(),
            eraObj.toObject().value("eraImagePath").toString(),
            QDate::fromString(eraObj.toObject().value("lastUpdate").toString(),
                              "yyyy-MM-dd"));
...
void JsonParser::prepareArtsArray() {
  ...
  for (auto artObj : artsArray) {
    QJsonArray m_artAuthorsNames =
        artObj.toObject().value("artAuthors").toArray();
    std::vector<Author> artAuthors = getArtAuthors(m_artAuthorsNames);

    Art art(artObj.toObject().value("artEra").toString(),
            artObj.toObject().value("artPath").toString(),
            artObj.toObject().value("artInfo").toString(),
            artObj.toObject().value("artName").toString(),
            artObj.toObject().value("isDomestic").toBool(),
            QDate::fromString(artObj.toObject().value("lastUpdate").toString(),
                              "yyyy-MM-dd"),
            artAuthors);
...
void JsonParser::prepareAuthorsArray() {
  ...
  for (auto authorObj : authorsArray) {
    Author author(
        authorObj.toObject().value("authorName").toString(),
        authorObj.toObject().value("authorImagePath").toString(),
        authorObj.toObject().value("authorInfo").toString(),
        QDate::fromString(authorObj.toObject().value("lastUpdate").toString(),
                          "yyyy-MM-dd"));
...
std::vector<Author> JsonParser::getArtAuthors(
    const QJsonArray& artAuthorsNames) {
  std::vector<Author> authors;

  for (auto artAuthorsObj : artAuthorsNames) {
    std::set<Author>::iterator author =
        m_authorsArray.find(Author(artAuthorsObj.toString(), "", "", QDate()));

    if (author != m_authorsArray.end()) authors.push_back(*author);
  }
  return authors;
}
