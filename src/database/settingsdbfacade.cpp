...
SettingsDBFacade::SettingsDBFacade(QObject* parent)
  : DBFacade(parent)
  , DefaultSettings({ { "path", QStandardPaths::standardLocations(QStandardPaths::HomeLocation)[0] },
                      { "sound", "none" },
                      { "data", "2019-03-07" } })
{
  if (false == m_db.tables().contains("settings"))
  {
    ...
    {
      exec(tr("INSERT INTO settings (property, value) VALUES (") + qs(setting.first) + ", " + qs(setting.second) +
           ");");
    }
  }
...
void SettingsDBFacade::set(const Settings& setting)
{
  try
  {
    exec(tr("UPDATE settings SET value = ") + qs(setting.value) + " WHERE property = " + qs(setting.property));
  }
  catch (...)
  {
    return;
