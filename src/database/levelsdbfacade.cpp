...
LevelsDBFacade::LevelsDBFacade(QObject* parent) : DBFacade(parent) {
  ...
  m_arts_model->setRelation(
      3, QSqlRelation("authors", "authorName", "authorName"));
...
Art LevelsDBFacade::randomArt() {
  exec(
      tr("SELECT era, path, info, name, domestic, lastUpdate FROM arts ORDER "
         "BY RANDOM() LIMIT 1;"));
  m_arts_model->select();
  m_query->first();
...
Art LevelsDBFacade::randomArt(Era& era_) {
  exec(tr("SELECT era, path, info, name, domestic, lastUpdate FROM arts WHERE "
          "era=") +
       qs(era_.eraName) + tr("ORDER BY RANDOM() LIMIT 1;"));
  m_arts_model->select();
  m_query->first();
...
std::set<Author> LevelsDBFacade::getAllAuthors() {
  ...
  exec(
      tr("SELECT authorName, authorImagePath, authorInfo, lastUpdate from "
         "authors;"));
  m_authors_model->select();
  while (m_query->next()) {
    authors.insert(
        Author(m_query->value(0).toString(), m_query->value(1).toString(),
               m_query->value(2).toString(), m_query->value(3).toDate()));
  }
...
std::vector<Art> LevelsDBFacade::arts() {
  ...
  while (m_query->next()) {
    ...
    arts.push_back(Art(era, path, info, name, domestic, lastUpdate,
                       std::vector<Author>()));
  }
...
void LevelsDBFacade::fillArtsAuthors(std::vector<Art>& arts) {
  for (auto& art : arts) art.artAuthors = getAuthorsOfArt(art.imgName);
}
...
std::vector<Author> LevelsDBFacade::getAuthorsOfArt(const QString& artName) {
  ...
  exec(tr("SELECT authorName, authorImagePath, authorInfo, lastUpdate from "
          "authors JOIN artsAuthors ON "
          "authors.authorName=artsAuthors.artAuthor WHERE artname=") +
       qs(artName) + ";");
  m_authors_model->select();
  while (m_query->next()) {
    Author author(m_query->value(0).toString(), m_query->value(1).toString(),
                  m_query->value(2).toString(), m_query->value(3).toDate());
...
std::set<Era> LevelsDBFacade::getEras() {
  ...
  m_eras_model->select();
  while (m_query->next()) {
    Era era(m_query->value(0).toString(), m_query->value(1).toString(),
            m_query->value(2).toDate());
...
std::set<Art> LevelsDBFacade::getArts() {
  ...
  while (m_query->next()) {
    ...
    arts.insert(Art(era, path, info, name, domestic, lastUpdate,
                    std::vector<Author>()));
  }
  fillArtsAuthors(arts);
...
void LevelsDBFacade::save(const Art& art) {
  try {
    exec(tr("INSERT INTO arts (era, path, info, name, domestic, lastUpdate) "
            "VALUES (") +
         qs(art.eraName) + ", " + qs(art.imgPath) + ", " + qs(art.imgInfo) +
         ", " + qs(art.imgName) + ", " + qs(art.domestic ? "1" : "0") + ", " +
...
void LevelsDBFacade::save(const Author& author) {
  try {
    exec(tr("INSERT INTO authors (authorName, authorImagePath, authorInfo, "
            "lastUpdate) VALUES (") +
         qs(author.authorName) + ", " + qs(author.imgPath) + ", " +
...
void LevelsDBFacade::addArtAuthor(const QString authorName,
                                  const QString artName) {
  try {
    exec(tr("INSERT INTO artsAuthors (artName, artAuthor) VALUES (") +
         qs(artName) + ", " + qs(authorName) + ");");
  } catch (...) {
    return;
  }
}
...
void LevelsDBFacade::checkStatistic(StatisticRecord& statisticRecord) {
  exec(tr("SELECT eraName, artName, worstTime, bestTime, mode FROM statistic "
          "WHERE artName =") +
       qs(statisticRecord.artName) + tr("AND mode =") +
...
void LevelsDBFacade::addStatistic(const StatisticRecord& statisticRecord) {
  try {
    exec(tr("INSERT INTO statistic (eraName, artName, worstTime, bestTime, "
            "mode) VALUES (") +
         qs(statisticRecord.eraName) + ", " + qs(statisticRecord.artName) +
...
void LevelsDBFacade::updateStatistic(const StatisticRecord& statisticRecord) {
  try {
    exec("UPDATE statistic SET worstTime=" + qs(statisticRecord.worstTime) +
         ", bestTime= " + qs(statisticRecord.bestTime) +
         +" WHERE artName=" + qs(statisticRecord.artName) +
...
std::vector<StatisticsTableItem> LevelsDBFacade::getAllStatistic() {
  ...
  exec(
      tr("SELECT eraname, MAX(worsttime), MIN(besttime), COUNT(eraname), "
...
void LevelsDBFacade::loadLevels(const QByteArray& jsonData) {
  ...
  for (; erasIt != eras.end(); ++erasIt) save(*erasIt);
...
void LevelsDBFacade::update(const Era& era) {
  ...
  if (era.imgPath != previousFilePath) deleteFile(previousFilePath);

  try {
    exec("UPDATE eras SET id=" + qs(era.eraName) +
         ", eraImagePath= " + qs(era.imgPath) +
         ", lastUpdate=" + qs(era.lastUpdate.toString("yyyy-MM-dd")) +
         " WHERE id=" + qs(era.eraName) + ";");
  } catch (...) {
    return;
  }
}
...
void LevelsDBFacade::update(const Art& art) {
  ...
  QString previousFilePath = m_query->value(0).toString();
  if (art.imgPath != previousFilePath) deleteFile(previousFilePath);

  try {
    exec("UPDATE arts SET era=" + qs(art.eraName) +
         ", path= " + qs(art.imgPath) + ", info=" + qs(art.imgInfo) +
         ", name=" + qs(art.imgName) +
         ", domestic=" + qs(art.domestic ? "1" : "0") +
         ", lastUpdate=" + qs(art.lastUpdate.toString("yyyy-MM-dd")) +
...
void LevelsDBFacade::update(const Author& author) {
  ...
  QString previousFilePath = m_query->value(0).toString();
  if (author.imgPath != previousFilePath) deleteFile(previousFilePath);

  try {
    exec("UPDATE authors SET authorName=" + qs(author.authorName) +
         ", authorImagePath= " + qs(author.imgPath) +
         ", authorInfo=" + qs(author.authorInfo) +
         ", lastUpdate=" + qs(author.lastUpdate.toString("yyyy-MM-dd")) +
