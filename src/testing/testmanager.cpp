    ...
    if (!eraArts.empty()) {
      ...
      for (auto& eraArt : eraArts) {
        ...
        questionsAndAnswers.push_back(
            QuestionWrapper(dragItem, eraDropItem, true, isDomestic));
        ...
        for (auto& artAuthor : eraArt.artAuthors) {
          ...
          questionsAndAnswers.push_back(
              QuestionWrapper(dragItem, authorDropItem, false, isDomestic));
        }
      }

      availableForTestingEras.push_back(EraListModelItem(
          era.eraName, domesticFilesCount, internationalFilesCount, false,
          false, false, false));
      m_eraModule.push_back(
          eraModuleQuestionsWrapper(era, questionsAndAnswers));
    }
  }
  emit erasReady(availableForTestingEras, true);
}

/// \brief Подсчёт количества отечественных и зарубежных вопросов для выбранного
/// количества эпох
void TestManager::getNumberOfEraQuestions(
    int& eraQuestions, int& authorQuestions,
    std::vector<EraListModelItem>& selectedEras) {
  for (auto& selectedEra : selectedEras) {
    auto foundedEra =
        std::find(m_eraModule.begin(), m_eraModule.end(), selectedEra.eraName);
    for (auto& question : foundedEra->questionsAndAnswers) {
      if (selectedEra.domesticSelected && question.isDomestic)
        increaseQuestionCounter(question.isEraQuestion, eraQuestions,
                                authorQuestions);
  ...
  qDebug() << QString::number(eraQuestions) + " " +
                  QString::number(authorQuestions);
}

/// \details Для каждого элемента из списка результатов проверяется тип
/// вопроса(автор или эпоха) Следом происходит его поиск в первых элементах
/// общего списка вопросов, равных residueCount При нахождении вопроса в общем
/// списке, результаты заносятся в список и вопрос удаляется как из общего
/// списка, так и из списка результатов
void TestManager::findAnswers(std::vector<DropGridItem>& results,
                              int& quadResidue) {
  auto find = [&](bool findCorrectAnswers) {
    while (quadResidue != 0) {
      ...
      for (int quadQuestionsIndex = 0; quadQuestionsIndex < quadResidue;
           quadQuestionsIndex++, questionsIt++) {
        if (findCorrectAnswers ? ((dropIt->answerObjectName ==
                                   questionsIt->dragGridItem.dragItemName) &&
                                  dropIt->dropItemName ==
                                      questionsIt->dropGridItem.dropItemName)
                               : dropIt->dropItemName ==
                                     questionsIt->dropGridItem.dropItemName) {
          m_testResults.push_back(TestResultsItem(
              questionsIt->dropGridItem.dropItemName, dropIt->answerObjectName,
              questionsIt->dragGridItem.dragItemName));
...
void TestManager::fillQuestionsList(
    int requiredQuestionsCount, std::vector<EraListModelItem> selectedEras) {
  ...
  while (requiredQuestionsCount != 0) {
    ...
    auto it = find(m_eraModule.begin(), m_eraModule.end(),
                   selectedEras[randomEra].eraName);
    ...
    auto randomItemIterator =
        (it->questionsAndAnswers.begin() + questionsRandomShift);
    ...
    if (eraQuestionsQuad_.size() == questionItems)
      mergeQuestionsQuad(eraQuestionsQuad_, m_eraForTestQuestions,
                         requiredQuestionsCount);

    if (authorQuestionsQuad_.size() == questionItems)
      mergeQuestionsQuad(authorQuestionsQuad_, m_authorForTestQuestions,
                         requiredQuestionsCount);
  }
}

void TestManager::mergeQuestionsQuad(
    std::vector<TestManager::QuestionWrapper>& questionsQuad,
    std::vector<TestManager::QuestionWrapper>& questionsList,
    int& questionsCount) {
  questionsList.insert(questionsList.end(), questionsQuad.begin(),
                       questionsQuad.end());
  questionsQuad.clear();
  questionsCount -= questionItems;
}

void TestManager::startTesting(std::vector<EraListModelItem> selectedEras,
                               int buttonNumber) {
  ...
  if ((eraQuestions / questionItems + artQuestions / questionItems) <
      countOfQuestions) {
...
void TestManager::sendQuadToDndModels() {
  ...
  for (size_t index = 0; index < questionItems; index++) {
    QuestionWrapper questionWrapper = questionType
                                          ? m_eraForTestQuestions[index]
                                          : m_authorForTestQuestions[index];
...
void TestManager::increaseQuestionCounter(bool isEraQuestion,
                                          int& eraQuestionsCount,
...
void TestManager::pointToRightVector(
    std::vector<QuestionWrapper>::iterator& it) {
  if (m_currentTypeOfQuestionIsEra)
...
void TestManager::deleteItemFromRightVector(
    std::vector<QuestionWrapper>::iterator& it) {
  if (m_currentTypeOfQuestionIsEra)
