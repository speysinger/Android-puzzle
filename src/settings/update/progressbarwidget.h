...
/// Класс устанавливает пиковое значение, изменяет прогресс загрузки и
/// уведомляет об этом
class LoadHandler : public QObject {
  Q_OBJECT
  Q_PROPERTY(float progress READ progress NOTIFY progressChanged)
 public:
  void setMaxValue(int value) { m_numberOfUpdatedItems = value; }
  void incValue() {
    if (m_numberOfUpdatedItems != 0) {
      m_currentProgress++;
      ;
      emit progressChanged(ceil(
          (float(m_currentProgress) / float(m_numberOfUpdatedItems)) * 100));
  ...
  float progress() { return m_currentProgress; }
